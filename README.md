...
- **更新提示**：启动时自动检查 GitHub 有没有新版本，侧边栏会出现提示，点一下就能下载、安装、自动重启到新版，不用再手动去下载 DMG。

## 数据处理说明

- 只读取本机 `~/.claude` 和 `~/.codex` 目录下的 session 记录——项目路径、活跃状态、上下文占用等，用来在这里展示，**这部分数据不会上传到任何地方**。
- 重命名和书签保存在 Fleet 自己的本地文件（`~/Library/Application Support/Fleet/`），不会写回 Claude Code 或 Codex。
- Fleet 唯一的网络请求是启动时查一下 GitHub 最新 Release，用来提示新版本；不涉及任何 session 内容，代码里没有引入第三方网络库。
